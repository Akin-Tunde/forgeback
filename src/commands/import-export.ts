...
export async function handleExportConfirmation(
  ...
  try {
    ...
    if (!wallet) {
      ...
      return {
        response: "❌ Wallet not found. Please create or import a wallet first.",
      };
    }
    ...
    if (!confirmed) {
      session.currentAction = undefined;
      await session.save();
      console.log("[ExportConfirmation] Cancelled, session cleared for userId:", userId);
      return {
    ...
    await session.save();
    return {
      response: "❌ An error occurred while exporting your private key. Please try again later.",
      buttons: [],
    };
  }
}